...
let fullNames = runners.map(name => {
...
// Problem 1 - email addresses of everyone on the list

let emailAddresses = runners.map(name => {
  return `${name.email}`;
});
console.log(emailAddresses);

// Problem 2 - total contributions over 100

let donationsOver100 = runners.filter(donation => donation.donation > 100);
console.log(donationsOver100);

// Problem 3 - total donation price from people that contributed 100 or more

let ticketPriceOver100 = donationsOver100.reduce(function(
  accumulator,
  currentValue
) {
  return accumulator + currentValue.donation;
...
0);
console.log(ticketPriceOver100);